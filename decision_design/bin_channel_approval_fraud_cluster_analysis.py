...
OUTPUT_HTML = ROOT_DIR / "bin_channel_approval_fraud_cluster_analysis.html"
OUTPUT_CSV = ROOT_DIR / "bin_channel_approval_fraud_clusters.csv"
...
def main():
    ...
    for label in sorted(merged["cluster_label"].unique()):
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers",
                name=name,
                marker=dict(color=colors[label], size=7, opacity=0.45),
                customdata=subset[
                    [
                        "UserCardBIN",
                        "Channel",
    ...
    stats = [
        ...
        ("Clusters Found", format_int(len(set(merged["cluster_label"])) - (1 if -1 in set(merged["cluster_label"]) else 0))),
        ...
        ("DBSCAN Params", f"eps={DBSCAN_EPS}, min_samples={DBSCAN_MIN_SAMPLES}"),
    ]

    observations = [
        "This DBSCAN run uses the raw approval and fraud gap pair on the high-volume population only.",
    ...
    subtitle = (
        ...
        f"<strong>Scope:</strong> DBSCAN clustering of approval and fraud gaps for high-volume nodes with more than {HIGH_VOLUME_MIN_ROWS} rows."
    )

    html_content = build_page(
        title="BIN-Channel Approval-Fraud Cluster Analysis (DBSCAN)",
        subtitle=subtitle,
